...
To reconnect everything immediately, enable **Settings → General → Session Persistence → Restore and expand every project on launch**. Macterm starts the restored panes in a staggered background batch, without switching projects, and expands every project in the sidebar so their tabs and activity are visible. This can open many shells or SSH connections at once, so it is off by default.
